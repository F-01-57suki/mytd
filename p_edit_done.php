...
<html lang="ja">
  ...
  //エラーなしで処理
  if(isset($board_in)):
    $sql = mysqli_prepare($link,'INSERT INTO `board_tbl` (`username`,`b_name`) VALUES (?,?)');
    mysqli_stmt_bind_param($sql,'ss',$_SESSION['username'],$board_in);
    mysqli_stmt_execute($sql);
    mysqli_stmt_close($sql);
    //$b_idに追加したid入れる
  ...
  if(isset($cont) and isset($b_id) and isset($l_id)):
    //ボードの変更なし
    if($b_id === $_POST['b_idnow']):
      $sql = mysqli_prepare($link,'UPDATE `postit_tbl` SET `l_id`=?,`cont`=? WHERE `p_id`=?');
      mysqli_stmt_bind_param($sql,'sss',$l_id,$cont,$_POST['p_idnow']);
      mysqli_stmt_execute($sql);
      mysqli_stmt_close($sql);
      //ラベルチェック
      $sql = mysqli_prepare($link,"SELECT `l_id` FROM `postit_tbl` WHERE `l_id`=?");
      mysqli_stmt_bind_param($sql,'s',$_POST['l_idnow']);
      mysqli_stmt_execute($sql);
      mysqli_stmt_store_result($sql);
      if(mysqli_stmt_num_rows($sql) === 0):
        mysqli_stmt_close($sql);
      ...
      mysqli_stmt_bind_param($sql,'ss',$_SESSION['username'],$_POST['p_idnow']);
      mysqli_stmt_execute($sql);
      mysqli_stmt_close($sql);
      ...
      mysqli_stmt_bind_param($sql,'s',$_POST['b_idnow']);
      mysqli_stmt_execute($sql);
      mysqli_stmt_store_result($sql);
      if(mysqli_stmt_num_rows($sql) === 0):
        mysqli_stmt_close($sql);
